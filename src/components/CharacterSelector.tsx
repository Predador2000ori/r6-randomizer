...
const CharacterSelectorSquad: React.FC = () => {
	...
	const [selectedCharacters, setSelectedCharacters] = useState<CharacterCardProps[]>([{
		name: "unknown",
		imageUrl: "https://placehold.co/150x150/EEE/31343C?font=lato&text=Attackers:%20Return%0ADefenders:%20Space", //https://placehold.co/
		weapons: {
	...
	return (
		<div className="character-selector">
			<div className="quantdiv">
				<label htmlFor="Quantity" aria-label="Quantity of characters">Quantity:</label>
				<input
					type="number"
					id="Quantity"
					value={quantity}
			...
			>
				<div>
					<svg 
						...
						onKeyDown={(e) => (e.key === ' ' || e.key === 'Enter') && handleSelectCharacters('attacker')}
						tabIndex={0}
						role="button"
						aria-label="Select Random Attacker"
					...
					<svg
						...
						onKeyDown={(e) => (e.key === ' ' || e.key === 'Enter') && handleSelectCharacters('defender')}
						tabIndex={0}
						role="button"
						aria-label="Select Random Defender"
